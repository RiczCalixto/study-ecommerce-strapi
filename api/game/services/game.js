...
async function setImage({ image, game, field = "cover" }) {
  const url = `https:${image}_bg_crop_1680_655.jpg`;
...
module.exports = {
  populate: async (params) => {
    const gogApirUrl = `https://www.gog.com/games/ajax/filtered?mediType=game&page=1&sort=popularity`;

    const {
      data: { products },
    } = await axios.get(gogApirUrl);
